...
#include "shotcrete_selector/shotcretePointsPublisher.h"
...
namespace rviz_plugin_shotcrete_points_publisher
{
shotcretePointsPublisher::shotcretePointsPublisher()
{
    updateTopic();
}
...
void shotcretePointsPublisher::updateTopic()
{
    nh_.param("frame_id", tf_frame_, std::string("/base_link"));
    rviz_cloud_topic_ = std::string("/shotcrete_selected_points");
    ...
    pc_subs_ =  nh_.subscribe(subs_cloud_topic_.c_str(),1,&shotcretePointsPublisher::PointCloudsCallback, this);

    ROS_INFO_STREAM_NAMED("SelectedPointsPublisher.updateTopic", "Publishing rviz shotcrete selected points on topic " <<  nh_.resolveName (rviz_cloud_topic_) );//<< " with frame_id " << context_->getFixedFrame().toStdString() );
...
void shotcretePointsPublisher::PointCloudsCallback(const sensor_msgs::PointCloud2ConstPtr &pc_msg)
{
...
int shotcretePointsPublisher::processKeyEvent( QKeyEvent* event, rviz::RenderPanel* panel )
{
...
int shotcretePointsPublisher::processMouseEvent( rviz::ViewportMouseEvent& event )
{
...
int shotcretePointsPublisher::_processSelectedAreaAndFindPoints()
{
    rviz::SelectionManager* sel_manager = context_->getSelectionManager();
    rviz::M_Picked selection = sel_manager->getSelection();
...
int chil = child->numChildren();
    ...
    pcl::PointCloud<pcl::PointXYZI>::Ptr selected_points_pcl(new pcl::PointCloud<pcl::PointXYZI>());
...
int shotcretePointsPublisher::_publishAccumulatedPoints()
{
    ROS_INFO_STREAM_NAMED("SelectedPointsPublisher._processSelectedAreaAndFindPoints",
...
PLUGINLIB_EXPORT_CLASS( rviz_plugin_shotcrete_points_publisher::shotcretePointsPublisher, rviz::Tool )
