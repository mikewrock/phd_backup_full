         ...
         phd::simple_trajectory_service::Response &res)
{
	...
	pcl::PointXYZI preva1, preva2, prevb1, prevb2;
	...
	while(target_pt.z < maxX.z && target_pt.z  < MAX_HEIGHT){
		...
		target_pts.push_back(target_pt);
		if(target_pt.z> maxX.z || target_pt.z > MAX_HEIGHT || (hdir_vector.x == preva1.x && dir_vector.x == preva2.x && hdir_vector.z == preva1.z && dir_vector.z == preva2.z && hdir_vector.y == preva1.y && dir_vector.y == preva2.y)){
			ROS_INFO("Breaking");
			break;
		}
