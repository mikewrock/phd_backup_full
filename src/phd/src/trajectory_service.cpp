...
phd::trajectory_array calc_points(float z_val, float P1x, float P1y, float zmax){
	...
	ROS_INFO("zval %f, zmax %f",z_val,zmax);
		...
		if(DEBUG) ROS_INFO("DLine Sorted %d - points",sorted_line.points.size());
		sorted_array.sections.push_back(sorted_line);
		z_val+=HEIGHT_STEP;
		dir = dir * -1;
	}
	if(DEBUG) ROS_INFO("Done making lines %d - lines",sorted_array.sections.size());
	...
	if(DEBUG) ROS_INFO("Done making chunks %d - chunks",t_array.sections.size());
	chunk.points.clear();
	bool complete = false;
