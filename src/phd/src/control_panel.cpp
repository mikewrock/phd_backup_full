...
void QNode::ascan(std::string filename, std::string markername, int cloud_num){
	...
	if (pcl::io::loadPCDFile<pcl::PointXYZI> (filename.c_str(), *cloud) == -1) PCL_ERROR ("Couldn't read file\n");
		...
		if(doc_client.call(doc_srv)){
			...
			ofstream myfile;
			std::stringstream fs;
			fs << "/home/mike/analyzed/multiposes.csv";
			myfile.open (fs.str().c_str(), std::ios::out|std::ios::app);
			ROS_INFO("Response: %lu", doc_srv.response.clouds.size());
			for(int sctr = 0; sctr < doc_srv.response.clouds.size(); ++ sctr){
				pf.str("");
				pf << markername << cloud_num  << "localized_V" << sctr << "-" << doc_srv.response.clouds[sctr].id << ".pcd";
				//Tell pcl what the 4th field information is
				doc_srv.response.clouds[sctr].cloud_out.fields[3].name = "intensity";
				pcl::fromROSMsg(doc_srv.response.clouds[sctr].cloud_out, *save_cloud);
				if(DEBUG) ROS_INFO("Saving to %s",pf.str().c_str());
				if(save_cloud->size() > 0) pcl::io::savePCDFileASCII (pf.str().c_str(), *save_cloud);
				myfile << CLOUD << cloud_ctr << "V_" << sctr << ",";
				for(int i = 0; i <16; ++i){
					//if(i%4 == 0) myfile << std::endl;
					myfile << doc_srv.response.clouds[sctr].transform_mat[i] << ",";
				}
				myfile << "Marker " << doc_srv.response.clouds[sctr].id << " at:," << doc_srv.response.clouds[sctr].x << "," << doc_srv.response.clouds[sctr].y << "," << doc_srv.response.clouds[sctr].z << ","<< doc_srv.response.clouds[sctr].id << ",";	
				//Call the localization service
				loc_srv.request.cloud_in = doc_srv.response.clouds[sctr].cloud_out;
				loc_srv.request.homing = false;
				mn.str("");
				mn << "/home/mike/processed/tracker/";
				loc_srv.request.marker_file = mn.str();
				if(loc_client.call(loc_srv)){
					ROS_INFO("Found tracker");	
					}
				myfile << std::endl;
